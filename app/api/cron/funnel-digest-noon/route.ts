...
export async function GET(request: NextRequest) {
  if (!isCronAuthorized(request)) {
    log.warn(LOG_SCOPE.CRON_FUNNEL_NOON, "Unauthorized cron request");
    return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  }
  const timeLabel = "12:00 PM CT";
  try {
    log.info(LOG_SCOPE.CRON_FUNNEL_NOON, "Starting run", { timeLabel });
    ...
    log.info(LOG_SCOPE.CRON_FUNNEL_NOON, "Digest built", { rows: enriched.length, staging_entries: stagingMap.size });
    ...
    if (!ok) {
      log.error(LOG_SCOPE.CRON_FUNNEL_NOON, "Send failed", error);
      return NextResponse.json({ ok: false, error }, { status: 500 });
    }
    log.info(LOG_SCOPE.CRON_FUNNEL_NOON, "Run complete", { count: enriched.length });
    return NextResponse.json({ ok: true, count: enriched.length });
  } catch (e) {
    log.error(LOG_SCOPE.CRON_FUNNEL_NOON, "Run failed", e);
